...
function OpenRoomMenu(property, owner)
	...
	table.insert(elements, {label = _U('wash_blackmoney'), value = 'wash_BlackMoney'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room', {
		...
		elseif data.current.value == 'wash_BlackMoney' then
					------------------------------------
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_', {
						...
						if amount == nil then
							TriggerEvent("esx_property:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('invalid_amount'))
							ESX.UI.Menu.CloseAll()
						else
							menu.close()
							TriggerEvent("esx_property:notify", "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('In_progress'))
							Citizen.Wait(10000)
...
RegisterNetEvent("esx_property:notify")
AddEventHandler("esx_property:notify", function(icon, type, sender, title, text)
    Citizen.CreateThread(function()
		Wait(1)
