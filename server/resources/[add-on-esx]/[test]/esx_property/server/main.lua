...
AddEventHandler('esx_property:washMoney', function(amount)
	WhiteningMoney(amount)

end)

function WhiteningMoney(amount)
	local xPlayer 		= ESX.GetPlayerFromId(source)
	...
	if amount > 0 and account.money >= amount then
		...
		--local washedMoney = math.floor(amount * (_percent + bonus))	
		local washedMoney = math.floor(amount - (_percent + bonus))	
		xPlayer.removeAccountMoney('black_money', amount)
		xPlayer.addMoney(washedMoney)
		
		TriggerClientEvent("esx_property:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('cash') .. amount .. '$ en '.. washedMoney .. '$')
		
	else
		TriggerClientEvent("esx_property:notify", source, "CHAR_LESTER_DEATHWISH", 1, _U('Notification'), false, _U('invalid_amount'))
	end
end
