...
local item_pool = {
    {item = "picnic", valuable = true},
    {item = "ketamine", valuable = true},
...
function GetNewRandomItem()
    Citizen.Trace("prend un item random ... dans la listes et check si il est positif ou non")
...
function GetNewRandomLocation()
    ...
    if #(pos) > area_size then
        ...
        Citizen.Trace("Prend une position random dans la listes ....")
...
function GenerateNewTarget()
    ...
    Citizen.Trace("génére une nouvelle cible")
end
...
AddEventHandler("prospecting-example:activateProspecting", function()
    ...
    Citizen.Trace("Prospecting.StartProspecting(player)")
end)
...
CreateThread(function()
    ...
    Citizen.Trace("set de la dificulté")
    -- Add a list of targets
    -- Each target needs an x, y, z and data entry
    Prospecting.AddTargets(locations)
    Citizen.Trace("AddTarget Location")
    -- Generate 10 random extra targets
    for n = 0, 10 do
        GenerateNewTarget()
        Citizen.Trace("GenerateNewTarget")
    end

    -- The player collected something
    Prospecting.SetHandler(function(player, data, source, x, y, z)
    ...
    Prospecting.OnStart(function(player)
        ...
        Citizen.Trace("Started prospecting ...")
    end)
    ...
    Prospecting.OnStop(function(player, time)
        ...
        Citizen.Trace("Stopped prospecting ...")
    end)
end)
...
--    local itemQuantity = xPlayer.getInventoryItem(data.item).count
