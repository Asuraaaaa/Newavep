...
local function HarvestCoke(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		if PlayersHarvestingCoke[source] == true then
			...
			if coke >= 100 then -- and coke.count >= coke.limit then
...
--[[
		...
		if PlayersHarvestingMeth[source] == true then
			...
			if meth >= 100 then --if meth.limit ~= -1 and meth.count >= meth.limit then
...
local function HarvestMethlab(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingMethlab[source] == true then
			...
			local methlab = xPlayer.getInventoryItem('methlab').count

			if methlab >= 100 then --.limit ~= -1 and methlab.count >= methlab.limit then
...
local function HarvestAcetone(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingAcetone[source] == true then
			...
			local _source = source
			local xPlayer = ESX.GetPlayerFromId(_source)
			local acetone = xPlayer.getInventoryItem('acetone').count

			if acetone >= 100 then-- -1 and acetone.count >= acetone.limit then
...
local function HarvestLithium(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingLithium[source] == true then
			...
			if lithium >= 100 then -- ~= -1 and lithium.count >= lithium.limit then
...
local function HarvestKetamine(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingKetamine[source] == true then
			...
			local ketamine = xPlayer.getInventoryItem('ketamine').count

			if ketamine >= 100 then --~= -1 and ketamine.count >= ketamine.limit then
...
local function HarvestWeed(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingWeed[source] == true then
			...
			if weed >= 100 then --.limit ~= -1 and weed.count >= weed.limit then
...
local function HarvestEcstasy(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingEcstasy[source] == true then
			...
			if ecstasy >= 100 then--.limit ~= -1 and ecstasy.count >= ecstasy.limit then
...
local function HarvestOpium(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingOpium[source] == true then
			...
			if opium >= 100 then --~= -1 and opium.count >= opium.limit then
...
local function HarvestBillet(source)
	...
	SetTimeout(Config.TimeToFarm, function()
		...
		if PlayersHarvestingBillet[source] == true then
			...
			local billet = xPlayer.getInventoryItem('billet').count

			if billet >= 100 then--.limit ~= -1 and billet.count >= billet.limit then
