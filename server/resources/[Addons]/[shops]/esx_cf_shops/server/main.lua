...
AddEventHandler('esx_shops:buyItem', function(itemName, amount, zone)
	...
	if xPlayer.getMoney() >= price then
		...
		if xPlayer.canCarryItem(itemName, amount) then
			TriggerClientEvent('esx:showNotification', _source, 'player_cannot_hold')
		else
			xPlayer.removeMoney(price)
			xPlayer.addInventoryItem(itemName, amount)
			TriggerClientEvent('esx:showNotification', _source, 'bought', amount, itemLabel, ESX.Math.GroupDigits(price))
		end
	else
		local missingMoney = price - xPlayer.getMoney()
		TriggerClientEvent('esx:showNotification', _source, 'not_enough', ESX.Math.GroupDigits(missingMoney))
	end
end)